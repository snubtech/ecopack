/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - MaterialPropertyController (라이브러리 > 소재물성)
 * ==============================================================================
 * 
 * 1. 담당 범위
 *    - 소재물성 화면이 쓰는 조회 API 입니다. 대상 테이블은 if001(소재/물성 데이터 목록) 입니다.
 * 
 * 2. GetFilters — 조회조건 셀렉트 목록
 *    - 포장차수·적용소재·사용환경·포장재구분·소재의구성·성능항목·단위 7종을 한 번에 돌려줍니다.
 *    - if001 을 코드/코드명으로 묶어(GroupBy) 중복을 없애고, 코드가 빈 행은 제외합니다.
 *    - 화면 진입 시 요청을 한 번으로 끝내려고 셀렉트마다 엔드포인트를 나누지 않았습니다.
 * 
 * 3. GetList — 목록 조회
 *    - 코드 7종으로 걸러내고, 키워드는 화면에 보이는 텍스트 컬럼들에 부분일치로 찾습니다.
 *    - 포장차수 → 적용소재 → 성능항목 순으로 정렬한 뒤 페이지 단위로 잘라 돌려줍니다.
 * 
 * 4. 공통 규칙
