/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - SecondaryTdController (기술문서)
 * ==============================================================================
 * 
 * 1. 담당 범위
 *    - 2차 기술문서 화면이 쓰는 API 입니다. 대상 테이블은 secondary_td(2차포장기술서기본) 입니다.
...
{
    /// <summary>
    /// 2차포장 기술문서(secondary_td / 기술문서 모듈 A) 화면용 API.
    /// 라우트: api/SecondaryTd
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    public class SecondaryTdController : ControllerBase
    {
        ...
        public SecondaryTdController(AppDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        ...
        // ─────────────────────────────────────────────────────────────
        // GET: api/SecondaryTd/Get?prjId=xxx
        ...
        public async Task<IActionResult> Get([FromQuery] string prjId, [FromQuery] string? repCustId)
        {
            ...
            try
            {
                var entity = await _context.SecondaryTd
                    .AsNoTracking()
        ...
        // ─────────────────────────────────────────────────────────────
        // POST: api/SecondaryTd/Save
        // 신규/수정 통합 저장(Upsert). 저장 시 lastWrtDtm 을 현재 타임스탬프로 갱신한다.
        // ─────────────────────────────────────────────────────────────
        [HttpPost("Save")]
        public async Task<IActionResult> Save([FromBody] SecondaryTdDto dto, [FromQuery] string? repCustId)
        {
            ...
            try
            {
                ...
                if (isNew)
                {
                    entity = new SecondaryTd
                    {
                        // 프론트가 기존 ID를 보내오면 그대로 쓰고, 없으면 채번
                        Pkg2TechDocId = string.IsNullOrWhiteSpace(dto.Pkg2TechDocId)
                            ? NewTechDocId(dto.PrjId)
                            : dto.Pkg2TechDocId
                    };
                    _context.SecondaryTd.Add(entity);
                }
                ...
                // 적합성 선언서가 참조하는 기술문서 번호(pkg2TechDocId)와 절대 어긋나지 않도록
                // 화면에서 무엇이 넘어오든 채번된 번호로 덮어쓴다. (화면에서도 읽기 전용)
                entity.DocNo = entity.Pkg2TechDocId;
        ...
        // ─────────────────────────────────────────────────────────────
        // GET: api/SecondaryTd/DownloadAtchDoc?prjId=xxx&slot=1
        ...
        public async Task<IActionResult> DownloadAtchDoc([FromQuery] string prjId, [FromQuery] int slot, [FromQuery] string? repCustId)
        {
            ...
            var entity = await _context.SecondaryTd.AsNoTracking().FirstOrDefaultAsync(x => x.PrjId == prjId);
            if (entity == null)
            {
        ...
        // ─────────────────────────────────────────────────────────────
        // POST: api/SecondaryTd/UploadAtchDoc   (multipart/form-data)
        ...
        // ─────────────────────────────────────────────────────────────
        // DELETE: api/SecondaryTd/DeleteAtchDoc?prjId=xxx&slot=1
        ...
        private static readonly PropertyInfo[] DtoProps =
            typeof(SecondaryTdDto).GetProperties(BindingFlags.Public | BindingFlags.Instance);
        ...
        private static void SetStringProperty(SecondaryTd entity, string name, string? value)
        {
        ...
        private static string? GetStringProperty(SecondaryTd entity, string name) =>
