/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - TertiaryDocController (적합성 선언서)
 * ==============================================================================
 * 
 * 1. 담당 범위
 *    - 3차 적합성 선언서 화면이 쓰는 API 입니다. 대상 테이블은 tertiary_doc(3차포장적합성선언서기본) 입니다.
 ...
 *    - tertiary_td 와 달리 컬럼이 모두 varchar(길이 제한)라, 화면에서도 입력 길이를 함께 제한합니다.
 * ==============================================================================
 */
...
namespace ecopack.Api.Controllers
{
    ...
    public class TertiaryDocController : ControllerBase
    {
        ...
        public TertiaryDocController(AppDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        ...
        // ─────────────────────────────────────────────────────────────
        // GET: api/TertiaryDoc/Get?prjId=xxx
        ...
        public async Task<IActionResult> Get([FromQuery] string prjId, [FromQuery] string? repCustId)
        {
            ...
            try
            {
                var entity = await _context.TertiaryDoc
                    .AsNoTracking()
                ...
                if (entity == null)
                {
                    ...
                    // 기술문서(tertiary_td)가 이미 있으면 그 ID를 미리 연결해 준다.
                    var techDocId = await _context.TertiaryTd
                        .AsNoTracking()
                        .Where(x => x.PrjId == prjId)
                        .Select(x => x.Pkg3TechDocId)
                        .FirstOrDefaultAsync();
                    ...
                    return Ok(new
                    {
                        success = true,
                        isNew = true,
                        data = new TertiaryDocDto { PrjId = prjId, Pkg3TechDocId = techDocId }
                    });
                }
        ...
        public async Task<IActionResult> Save([FromBody] TertiaryDocDto dto, [FromQuery] string? repCustId)
        {
            ...
            try
            {
                ...
                if (isNew)
                {
                    entity = new TertiaryDoc
                    {
                        Pkg3DocId = string.IsNullOrWhiteSpace(dto.Pkg3DocId)
                            ? NewDocId(dto.PrjId)
                            : dto.Pkg3DocId
                    };
                    _context.TertiaryDoc.Add(entity);
                }
                ...
                if (string.IsNullOrWhiteSpace(entity.Pkg3TechDocId))
                {
                    entity.Pkg3TechDocId = await _context.TertiaryTd
                        .AsNoTracking()
        ...
        // ─────────────────────────────────────────────────────────────
        // GET: api/TertiaryDoc/DownloadEvdDoc?prjId=xxx&slot=1
        ...
        public async Task<IActionResult> DownloadEvdDoc([FromQuery] string prjId, [FromQuery] int slot, [FromQuery] string? repCustId)
        {
            ...
            var entity = await _context.TertiaryDoc.AsNoTracking().FirstOrDefaultAsync(x => x.PrjId == prjId);
            if (entity == null)
            {
        ...
        // ─────────────────────────────────────────────────────────────
        // POST: api/TertiaryDoc/UploadEvdDoc   (multipart/form-data)
            ...
            [FromQuery] string? repCustId)
        {
            ...
            try
            {
                var entity = await _context.TertiaryDoc.FirstOrDefaultAsync(x => x.PrjId == prjId);
                if (entity == null)
                {
        ...
        // ─────────────────────────────────────────────────────────────
        // DELETE: api/TertiaryDoc/DeleteEvdDoc?prjId=xxx&slot=1
        ...
        private static readonly PropertyInfo[] DtoProps =
            typeof(TertiaryDocDto).GetProperties(BindingFlags.Public | BindingFlags.Instance);
        ...
        private static readonly HashSet<string> SkipOnWrite =
            new(StringComparer.Ordinal) { nameof(TertiaryDocDto.Pkg3DocId), nameof(TertiaryDocDto.LastWrtDt) ,
            nameof(TertiaryDocDto.EvdDocNm1), nameof(TertiaryDocDto.EvdDocNm2), nameof(TertiaryDocDto.EvdDocNm3), nameof(TertiaryDocDto.EvdDocNm4), nameof(TertiaryDocDto.EvdDocNm5), nameof(TertiaryDocDto.EvdDocNm6), nameof(TertiaryDocDto.EvdDocNm7), nameof(TertiaryDocDto.EvdDocNm8) };

        private static TertiaryDocDto ToDto(TertiaryDoc entity)
        {
            var dto = new TertiaryDocDto();
        ...
        private static void ApplyDtoToEntity(TertiaryDocDto dto, TertiaryDoc entity)
        {
            foreach (var dp in DtoProps)
            {
        ...
        private static void SetStringProperty(TertiaryDoc entity, string name, string? value)
        {
        ...
        private static string? GetStringProperty(TertiaryDoc entity, string name) =>
