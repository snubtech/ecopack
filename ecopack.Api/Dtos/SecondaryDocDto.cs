/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - SecondaryDocDto (적합성 선언서 주고받기용 자료 묶음)
 * ==============================================================================
 * 
 * 1. 쓰임새
 *    - 적합성 선언서 화면과 서버가 주고받는 항목을 담습니다. secondary_doc 전체 컬럼과 1:1로 맞췄습니다.
 * 
 * 2. 타입을 모두 string? 으로 둔 이유
 *    - 화면의 입력 항목이 전부 글자이기 때문입니다. 빈 문자열은 null 로 저장됩니다.
 * 
 * 3. 함께 두는 자료
 *    - SecondaryEvdDocUploadResultDto : 근거문서(부속서)를 올린 결과(슬롯 번호·파일명·경로)를 돌려줍니다.
 * ==============================================================================
 */
namespace ecopack.Api.Dtos
{
    /// <summary>
    /// 2차포장 적합성선언서(secondary_doc) 화면 CRUD DTO.
    /// 화면의 입력 항목은 모두 텍스트로 다루므로 DTO는 전부 string? 으로 받는다.
    /// (빈 문자열 → null 로 저장, null → 요청에 없는 항목이므로 기존 값 유지)
    /// </summary>
    public class SecondaryDocDto
    {
        // ── 키 ────────────────────────────────────────────────
        /// <summary>2차포장 적합성선언서 ID. 신규 저장 시 서버에서 DOC-2-{yyyyMMddHHmmssfff} 로 채번</summary>
        public string? Pkg2DocId { get; set; }
        ...
        /// <summary>2차포장기술문서 고유 ID이다.</summary>
        public string? Pkg2TechDocId { get; set; }
        ...
        /// <summary>4.3. 우려물질 및 중금속 제한 적합의 총합행 하단 고정 내용이다.</summary>
    ...
    public class SecondaryEvdDocUploadResultDto
    {
