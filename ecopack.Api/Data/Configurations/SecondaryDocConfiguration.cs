/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - SecondaryDocConfiguration (secondary_doc 테이블 매핑)
 * ==============================================================================
 * 
 * 1. 하는 일
 *    - 엔티티 SecondaryDoc 을 실제 테이블 secondary_doc 에 이어 줍니다.
 *    - 실제 DB의 SHOW CREATE TABLE 결과를 그대로 옮겨 적었습니다(2026-09).
 * 
 * 2. 처음 만들어졌을 때 잘못돼 있던 점
 *    - secondary_td 와 같은 문제였습니다. 테이블명이 primarydoc 이고 컬럼명이 전부 소문자였습니다.
 * 
 * 3. 맞춘 내용
 *    - 테이블명 secondary_doc, 컬럼명은 실제와 같은 대소문자(pkg2DocId, soCHvyMetLmtCmplCntn1 …)
 ...
 * 
 * 4. 참고
 *    - 컬럼별 한글 설명은 엔티티 SecondaryDoc.cs 의 주석에 적혀 있습니다.
 * ==============================================================================
 */
...
{
    /// <summary>
    /// secondary_doc (2차포장적합성선언서기본) 테이블 매핑.
    /// 실제 DB의 SHOW CREATE TABLE 결과에 맞춰 작성됨 (2026-09):
    ///  - 테이블명: secondary_doc (언더스코어)
    ///  - PK: pkg2DocId varchar(50)
    ///  - 문자열 컬럼은 전부 varchar(길이 지정)
    ///  - sbstTot 은 DB DEFAULT '총합'
    ///  - lastWrtDt 는 date 타입, DB DEFAULT (curdate())
    /// 컬럼 정의(한글 설명)는 엔티티 SecondaryDoc.cs 의 XML 주석 참고.
    /// </summary>
    public class SecondaryDocConfiguration : IEntityTypeConfiguration<SecondaryDoc>
    {
        public void Configure(EntityTypeBuilder<SecondaryDoc> builder)
        {
            builder.ToTable("secondary_doc");
            builder.HasKey(e => e.Pkg2DocId);

            builder.Property(e => e.Pkg2DocId).HasColumnName("pkg2DocId").HasMaxLength(50);
            ...
            builder.Property(e => e.Pkg2TechDocId).HasColumnName("pkg2TechDocId").HasMaxLength(50);
