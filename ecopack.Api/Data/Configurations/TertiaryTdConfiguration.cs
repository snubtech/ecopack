/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - TertiaryTdConfiguration (tertiary_td 테이블 매핑)
 * ==============================================================================
 * 
 * 1. 하는 일
 *    - 엔티티 TertiaryTd 를 실제 테이블 tertiary_td 에 이어 줍니다.
 *    - 실제 DB의 SHOW CREATE TABLE 결과를 그대로 옮겨 적었습니다(2026-09).
 * 
 * 2. 처음 만들어졌을 때 잘못돼 있던 점
 *    - 테이블명이 primarytd 로 되어 있어 실제 테이블(tertiary_td)을 찾지 못했습니다.
 *    - 컬럼명도 전부 소문자로 적혀 있었고, 문자열 길이도 실제와 달랐습니다.
 * 
 * 3. 맞춘 내용
 *    - 테이블명 tertiary_td, 컬럼명은 실제와 같은 대소문자(prjId, prdExplPhrsCntn …)
 *    - 문자열은 text, 중량·비율은 decimal(10,2), BOM 수량은 int
 *    - lastWrtDtm 은 DB 기본값 CURRENT_TIMESTAMP 를 따릅니다.
 * 
 * 4. 참고
 *    - 컬럼별 한글 설명은 엔티티 TertiaryTd.cs 의 주석에 적혀 있습니다.
 * ==============================================================================
 */
...
{
    /// <summary>
    /// tertiary_td (3차포장기술서기본) 테이블 매핑.
    /// 실제 DB의 SHOW CREATE TABLE 결과에 맞춰 작성됨 (2026-09):
    ///  - 테이블명: tertiary_td (언더스코어)
    ///  - PK: pkg3TechDocId varchar(50)
    ///  - 문자열 컬럼은 전부 MySQL text
    ///  - 금액/중량류는 decimal(10,2), BOM 수량은 int
    ///  - lastWrtDtm 은 DB DEFAULT CURRENT_TIMESTAMP
    /// 컬럼 정의(한글 설명)는 엔티티 TertiaryTd.cs 의 XML 주석 참고.
    /// </summary>
    public class TertiaryTdConfiguration : IEntityTypeConfiguration<TertiaryTd>
    {
        public void Configure(EntityTypeBuilder<TertiaryTd> builder)
        {
            builder.ToTable("tertiary_td");
            builder.HasKey(e => e.Pkg3TechDocId);
