/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - primaryDoc API 클라이언트 (3차 적합성 선언서)
 * ==============================================================================
 * 
 * 1. 쓰임새
 *    - 3차 적합성 선언서 화면이 쓰는 서버 통신 묶음입니다. 대상 테이블은 tertiary_doc 입니다.
 *    - 기존 도메인 코드(projects.js)와 같이 axios 로 직접 호출합니다.
 * 
 * 2. 제공 함수
 *    - GetTertiaryDoc        : 프로젝트 한 건의 선언서를 받아옵니다. 작성 전이면 빈 문서가 옵니다.
 *    - SaveTertiaryDoc       : 신규/수정을 한 번에 처리합니다(Upsert).
 ...
 * 
 * 4. 알아둘 점
 *    - 문서 ID 채번(DOC-3-{타임스탬프})과 발행일 갱신은 서버가 처리합니다.
 *    - 같은 프로젝트의 3차 기술문서가 있으면 서버가 3차 기술문서 번호를 자동으로 연결합니다.
 * ==============================================================================
 */
...
/**
 * 1차포장 적합성선언서(tertiary_doc) API 클라이언트
 * - 백엔드 라우트: api/TertiaryDoc
 ...
 *   (이때 3차 기술문서가 이미 있으면 pkg1TechDocId가 미리 채워집니다)
 * @param {string} prjId 프로젝트 ID
 */
export async function GetTertiaryDoc(prjId) {
    // 로그인한 고객 ID를 함께 보내 본인 프로젝트의 문서인지 서버가 확인하게 한다
    const response = await axios.get('/api/TertiaryDoc/Get', { params: { prjId, repCustId: getCurrentCustomerId() } });
 ...
 * - 신규일 경우 pkg1DocId 를 DOC-3-{타임스탬프} 규칙으로 채번합니다.
 * @param {object} dto 화면 입력값 전체
 */
export async function SaveTertiaryDoc(dto) {
    // 저장도 본인 프로젝트인지 확인받는다
    const response = await axios.post('/api/TertiaryDoc/Save', dto, { params: { repCustId: getCurrentCustomerId() } });
    return response.data;
}
...
export async function UploadEvdDoc(prjId, slot, file) {
    ...
    const response = await axios.post('/api/TertiaryDoc/UploadEvdDoc', formData, {
...
export async function DeleteEvdDoc(prjId, slot) {
    const response = await axios.delete('/api/TertiaryDoc/DeleteEvdDoc', {
...
export function getEvdDocDownloadUrl(prjId, slot) {
    ...
    return `/api/TertiaryDoc/DownloadEvdDoc?${params.toString()}`;
}
