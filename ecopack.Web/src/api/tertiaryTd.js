/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - primaryTd API 클라이언트 (3차 기술문서)
 * ==============================================================================
 * 
 * 1. 쓰임새
 *    - 3차 기술문서 화면이 쓰는 서버 통신 묶음입니다. 대상 테이블은 tertiary_td 입니다.
 *    - 기존 도메인 코드(projects.js)와 같이 axios 로 직접 호출합니다.
 * 
 * 2. 제공 함수
 *    - GetTertiaryTd        : 프로젝트 한 건의 3차 기술문서를 받아옵니다. 작성 전이면 빈 문서가 옵니다.
 *    - SaveTertiaryTd       : 신규/수정을 한 번에 처리합니다(Upsert).
 ...
 * 
 * 4. 알아둘 점
 *    - 문서 ID 채번(TD-3-{타임스탬프})과 작성일시 갱신은 서버가 처리합니다.
 * ==============================================================================
 */
...
/**
 * 1차포장 3차 기술문서(tertiary_td) API 클라이언트
 * - 백엔드 라우트: api/TertiaryTd
 * - 기존 도메인 코드(projects.js)와 동일하게 axios 직접 호출 방식을 사용합니다.
 */

/**
 * 1. 3차 기술문서 조회
 * - 해당 프로젝트의 3차 기술문서를 가져옵니다.
 * - 아직 작성 전이면 isNew=true 와 빈 데이터를 돌려줍니다.
 * @param {string} prjId 프로젝트 ID
 * @returns {Promise<{success:boolean, isNew:boolean, data:object}>}
 */
export async function GetTertiaryTd(prjId) {
    // 로그인한 고객 ID를 함께 보내 본인 프로젝트의 문서인지 서버가 확인하게 한다
    const response = await axios.get('/api/TertiaryTd/Get', { params: { prjId, repCustId: getCurrentCustomerId() } });
    return response.data;
}

/**
 * 2. 3차 기술문서 저장 (신규/수정 통합 Upsert)
 * - 저장 시 서버가 lastWrtDtm 을 현재 타임스탬프로 갱신합니다.
 * - 신규일 경우 pkg1TechDocId 를 TD-3-{타임스탬프} 규칙으로 채번합니다.
 * @param {object} dto 화면 입력값 전체
 */
export async function SaveTertiaryTd(dto) {
    // 저장도 본인 프로젝트인지 확인받는다
    const response = await axios.post('/api/TertiaryTd/Save', dto, { params: { repCustId: getCurrentCustomerId() } });
    return response.data;
}
...
export async function UploadAtchDoc(prjId, slot, file) {
    ...
    const response = await axios.post('/api/TertiaryTd/UploadAtchDoc', formData, {
...
export async function DeleteAtchDoc(prjId, slot) {
    const response = await axios.delete('/api/TertiaryTd/DeleteAtchDoc', {
...
export function getAtchDocDownloadUrl(prjId, slot) {
    ...
    return `/api/TertiaryTd/DownloadAtchDoc?${params.toString()}`;
}
...
export async function UploadMfrDrw(prjId, file) {
    ...
    const response = await axios.post('/api/TertiaryTd/UploadMfrDrw', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        params: { repCustId: getCurrentCustomerId() },
    });
    return response.data;
...
export async function DeleteMfrDrw(prjId) {
    const response = await axios.delete('/api/TertiaryTd/DeleteMfrDrw', {
