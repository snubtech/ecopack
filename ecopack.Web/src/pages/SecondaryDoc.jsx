/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - SecondaryDoc 컴포넌트 (적합성 선언서)
 * ==============================================================================
 * 
 * 1. 화면 구성
 *    - 좌측 사이드바의 [DOC (2차 적합성 선언서)] 를 누르면 곧바로 작성 화면이 열립니다.
 *    - 어떤 프로젝트의 문서인지는 세션에 담긴 프로젝트 정보(currentPrjId)로 정해지므로,
 *      [프로젝트 현황] 에서 프로젝트를 고른 뒤 들어와야 저장까지 이어집니다.
 * 
 * 2. 초기 데이터 로드 (useEffect)
 *    - 세션의 프로젝트 ID로 서버에서 적합성선언서를 조회합니다.
 *    - 아직 작성 전이면 빈 문서를 받아 고정 문구(DEFAULTS)를 채우고 바로 편집 상태로 엽니다.
 *    - 같은 프로젝트의 기술문서가 이미 있으면 기술문서 번호가 미리 연결됩니다.
 *    - 비어 있는 제조자·담당자 항목은 로그인한 회원가입 정보(MEMBER_FIELDS)로 채웁니다.
 * 
 * 3. 표의 행 늘리고 줄이기 (ROW_TABLES / addRow / removeRow)
 *    - 중금속 시험결과·재질정보·부속서는 행을 더하거나 지울 수 있습니다.
 *    - 늘릴 수 있는 최대 행 수는 DB 컬럼 슬롯 개수로 정해집니다.
 * 
 * 4. 저장 (handleSave)
 *    - 화면의 모든 항목을 한 번에 보내 신규/수정을 함께 처리합니다(Upsert).
 *    - 신규면 서버가 DOC-2-{타임스탬프} 규칙으로 문서 ID를 채번하고,
 *      저장할 때마다 발행일(lastWrtDt)을 오늘 날짜로 갱신합니다.
 * 
 * 5. 근거문서 (handleUpload / handleDeleteFile / handleDeleteRow)
 *    - 부속서 A~H 슬롯에 파일을 올리면 서버가 파일을 보관하고 문서명·경로를 컬럼에 기록합니다.
 * 
 * 6. 문서 추출 (handleExportPdf / handleExportDocx)
 *    - 화면에서는 입력하기 편한 형태로 두고, 추출할 때만 문서에 어울리는 형태로 바꿉니다.
 *      (4.3 결과 문구, 7. 대표자/책임자의 '직책 이름' 표기가 그런 경우입니다)
 * 
 * 7. 화면 렌더링 (JSX)
 *    - PDF 원문과 같은 순서로 7개 절을 위에서 아래로 배치하고,
 *      상단 액션 바는 스크롤해도 고정되게 했습니다.
 * ==============================================================================
 */
...
const SINGLE_KEYS = [
    ...
    'prjfNm', 'prjId', 'pkg2TechDocId', 'revNo', 'cntryNm', 'dsgnTypeNm',
    'docPhrsCntn',
...
// 메인 화면
// ─────────────────────────────────────────────────────────────
export default function SecondaryDoc() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const res = await GetSecondaryDoc(prjId);
                if (!alive) return;
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            // 발행일(lastWrtDt)는 서버가 저장할 때 직접 기록한다.
            // 화면의 폼은 모든 값을 글자로 다루기 때문에 그대로 보내면 빈 문자열('')이 날짜 자리에 들어가
            // 서버가 날짜로 바꾸지 못해 요청이 거절된다. 그래서 보낼 때 빼 준다.
            const { lastWrtDt: _omit, ...payload } = form;
            void _omit;

            const res = await SaveSecondaryDoc({ ...payload, prjId });
...
<style>
  ...
  .doc-matinfo-row span { margin-right: 6pt; }
        ...
        a.href = url;
        a.download = `2차적합성선언서_${form.prjfNm || prjId || 'secondary_doc'}.doc`;
        ...
        <div className="td-page">
            ...
            <div className="td-toolbar td-noprint">
                <div className="td-toolbar-info">
                    <strong>DOC (2차 적합성 선언서)</strong>
                    <span className="td-badge">{isNew ? '신규 작성' : '저장됨'}</span>
                    {form.pkg2DocId && <span className="td-docid">{form.pkg2DocId}</span>}
            ...
            <div className="td-doc" id="td-doc" ref={docRef}>
                <h1 className="td-title doc-title">PPWR 적합성 선언서 (2차)</h1>
                ...
                <table className="td-table td-table-kv td-table-plain">
                    <tbody>
                        ...
                        <tr><th>3. 기술문서 번호</th><td>{input('pkg2TechDocId')}</td></tr>
