/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - MaterialProperty 컴포넌트 (라이브러리 > 소재물성)
 * ==============================================================================
 * 
 * 1. 초기 데이터 로드 (useEffect)
 *    - 컴포넌트가 처음 실행되면 조회조건 셀렉트 7종(포장차수·적용소재·사용환경·포장재구분·
 *      소재의구성·성능항목·단위)을 서버에서 한 번에 받아옵니다.
 *    - 이어서 조건 없이 전체 목록의 첫 페이지를 조회합니다.
 * 
 * 2. 조회 조건 관리 (condition / appliedRef)
 *    - 사용자가 고르는 값은 condition 상태에 담기고, 실제 조회에 쓰인 값은 appliedRef 에 따로 둡니다.
 *    - 조건만 바꿔서는 목록이 갱신되지 않고 [조회] 를 눌러야 반영되도록 나눈 것입니다.
 * 
 * 3. 목록 조회 (fetchList)
 *    - 확정된 조건과 페이지 번호를 서버에 보내 목록과 전체 건수를 받아옵니다.
 *    - 비어 있는 조건은 '전체' 로 보고 서버에서 조건을 건너뜁니다.
 * 
 * 4. 엑셀 내려받기 (handleExcel)
 *    - 현재 화면에 보이는 페이지가 아니라, 조회조건에 해당하는 전체 건을 다시 받아
 *      xlsx 파일로 만들어 내려받습니다. (pageSize 를 0 으로 보내면 서버가 전체를 반환)
 * 
 * 5. 화면 렌더링 (JSX)
