/**
 * ==============================================================================
 * [프로그램 전체 흐름 및 구조 요약] - TertiaryTd 컴포넌트 (기술문서)
 * ==============================================================================
 * 
 * 1. 화면 구성
 *    - 좌측 사이드바의 [TD (3차 기술문서)] 를 누르면 곧바로 작성 화면이 열립니다.
 *    - 어떤 프로젝트의 문서인지는 세션에 담긴 프로젝트 정보(currentPrjId)로 정해지므로,
 *      [프로젝트 현황] 에서 프로젝트를 고른 뒤 들어와야 저장까지 이어집니다.
 * 
 * 2. 초기 데이터 로드 (useEffect)
 *    - 세션의 프로젝트 ID로 서버에서 기술문서를 조회합니다.
 *    - 아직 작성 전이면 빈 문서를 받아 고정 문구(DEFAULTS)를 채우고 바로 편집 상태로 엽니다.
 *    - 비어 있는 제조사·담당자 항목은 로그인한 회원가입 정보(MEMBER_FIELDS)로 채웁니다.
 * 
 * 3. 표의 행 늘리고 줄이기 (ROW_TABLES / addRow / removeRow)
 *    - 재질구성·BOM·제품설계·재활용원칙·중금속·품질관리·첨부문서는 행을 더하거나 지울 수 있습니다.
 *    - 늘릴 수 있는 최대 행 수는 DB 컬럼 슬롯 개수(예: BOM 은 bomCmpnNm1~8)로 정해집니다.
 *    - 행을 지우면 아래 행 값을 위로 당겨 슬롯이 중간에 비지 않게 합니다.
 * 
 * 4. 저장 (handleSave)
 *    - 화면의 모든 항목을 한 번에 보내 신규/수정을 함께 처리합니다(Upsert).
 *    - 신규면 서버가 TD-3-{타임스탬프} 규칙으로 문서 ID를 채번하고,
 ...
 *    - 문서명은 확장자를 포함한 원본 파일명 그대로 들어갑니다.
 * 
 * 6. 문서 추출 (handleExportPdf / handleExportDocx)
 *    - PDF 는 브라우저 인쇄를 띄우고 인쇄용 CSS 로 사이드바와 버튼을 감춰 문서만 남깁니다.
 *    - DOCX 는 화면을 복사해 입력칸을 값 텍스트로 바꾼 뒤 Word 문서로 내려받습니다.
 * 
 * 7. 화면 렌더링 (JSX)
 *    - PDF 원문과 같은 순서로 12개 절을 위에서 아래로 배치하고,
 ...
 *  - [insert][컬럼명 : x] : tertiary_td 컬럼과 1:1로 묶인 입력 필드 (CRUD 대상)
...
// 메인 화면
// ─────────────────────────────────────────────────────────────
export default function TertiaryTd() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const res = await GetTertiaryTd(prjId);
                if (!alive) return;
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            // 작성일시(lastWrtDtm)는 서버가 저장할 때 직접 기록한다.
            ...
            const res = await SaveTertiaryTd({ ...payload, prjId });
...
<style>
  ...
  .td-row-label { font-weight: bold; }
        ...
        a.href = url;
        a.download = `3차기술문서_${form.prjfNm || prjId || 'tertiary_td'}.doc`;
        ...
        <div className="td-page">
            ...
            <div className="td-toolbar td-noprint">
                <div className="td-toolbar-info">
                    <strong>TD (3차 기술문서)</strong>
                    <span className="td-badge">{isNew ? '신규 작성' : '저장됨'}</span>
                    {form.pkg3TechDocId && <span className="td-docid">{form.pkg3TechDocId}</span>}
            ...
            <div className="td-doc" id="td-doc" ref={docRef}>
                <h1 className="td-title td-title-center">3차 기술문서</h1>
